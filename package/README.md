...
```sh
cargo install --path package --locked
```
...
cargo test --manifest-path package/Cargo.toml --locked
cargo clippy --manifest-path package/Cargo.toml --locked --all-targets -- -D warnings
```
